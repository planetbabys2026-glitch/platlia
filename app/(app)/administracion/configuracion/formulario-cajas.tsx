...
function ClaveDeSalidas({ puesta }: { puesta: boolean }) {
  const [estado, accion] = useActionState(guardarClaveGastos, ESTADO_INICIAL);
  const [quitado, quitar] = useActionState(quitarClaveGastos, ESTADO_INICIAL);
    ...
    <div className="space-y-3 rounded-xl border border-border/80 bg-card p-4">
      <div className="flex items-start gap-3">
        ...
        <div className="space-y-1">
          <h3 className="rotulo-seccion">Clave de salidas de dinero</h3>
          <p className="text-sm text-muted-foreground text-pretty">
            Se pide para registrar un gasto, un retiro o un ajuste negativo. La ponés
            y la cambiás solo vos: es la puerta por la que sale la plata del negocio.
          </p>
        </div>
      </div>
      ...
      {!puesta && (
        <Alert className="border-warning/40 bg-warning/10">
          <AlertDescription className="text-warning-soft">
            Todavía no hay clave. Cualquiera con acceso a la caja puede registrar una
            salida de dinero.
          </AlertDescription>
        </Alert>
      )}
      ...
      <form action={accion} className="space-y-3">
        ...
        {/* Solo cuando ya hay una: cambiarla sin conocer la vigente convertiría un
            descuido —una sesión abierta en un celular— en la llave de la plata. */}
        {puesta && (
          <div className="space-y-1.5">
            <Label htmlFor="claveActual">Clave actual</Label>
            <Input id="claveActual" name="claveActual" type="password" autoComplete="off" required />
          </div>
        )}

        <div className="grid gap-3 sm:grid-cols-2">
          <div className="space-y-1.5">
            <Label htmlFor="clave">{puesta ? "Clave nueva" : "Clave"}</Label>
            <Input id="clave" name="clave" type="password" autoComplete="new-password" required minLength={6} />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="claveRepetida">Repetila</Label>
            <Input
              id="claveRepetida"
              name="claveRepetida"
              type="password"
        ...
        <div className="border-t border-dashed border-border pt-3">
          {quitando ? (
            <form action={quitar} className="space-y-2">
              ...
              <Label htmlFor="claveParaQuitar">Escribí la clave actual para quitarla</Label>
              ...
              <p className="text-xs text-muted-foreground">
                Sin clave, las salidas de dinero se registran sin autorización.
              </p>
            </form>
          ) : (
            <Button
...
export function FormularioCajas({
  ...
  return (
    <div className="space-y-6">
      ...
      <ClaveDeSalidas puesta={claveSalidasPuesta} />
    </div>
  );
}
